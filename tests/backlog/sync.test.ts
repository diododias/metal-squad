...
`);
  ...
  it('marks a feature done in backlog.yaml so it stops showing up as pending', async () => {
    cwd = mkdtempSync(join(tmpdir(), 'msq-sync-'));
    writeFileSync(join(cwd, 'backlog.yaml'), `version: 2
repo: demo
defaults:
...
`);
    ...
    const { markFeatureStatus } = await import('../../src/core/backlog/sync.js');
    markFeatureStatus('feat-1', 'done', cwd);

    const updated = parse(readFileSync(join(cwd, 'backlog.yaml'), 'utf8')) as {
      epics: Array<{ features: Array<{ id: string; status?: string }> }>;
    };
    expect(updated.epics[0]?.features[0]?.status).toBe('done');
  });

  it('does nothing when the feature is absent from the backlog', async () => {
    cwd = mkdtempSync(join(tmpdir(), 'msq-sync-'));
  ...
  - id: epic-1
    ...
    features:
      - id: feat-1
        ...
        tasks: []
`;
    writeFileSync(join(cwd, 'backlog.yaml'), original);

    const { markFeatureStatus } = await import('../../src/core/backlog/sync.js');
    expect(() => markFeatureStatus('feat-missing', 'done', cwd)).not.toThrow();
    expect(readFileSync(join(cwd, 'backlog.yaml'), 'utf8')).toBe(original);
  });
});
