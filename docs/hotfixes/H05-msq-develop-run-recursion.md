...
- A skill `msq-develop` agora inclui no campo `spec` uma secao obrigatoria de **regras anti-recursao** que proibe explicitamente invocar `msq`, `node dist/index.js run`, `npm run dev` ou qualquer nested runner dentro de uma run em andamento.
- O passo "Preparar o backlog.yaml" da skill foi atualizado para exigir que o campo `spec` sempre contenha a proibicao de recursao como primeiro bloco, antes do escopo funcional.
- A secao DONT da skill foi estendida para cobrir explicitamente nested runners.

## Evidencia de implementacao

- `.agents/skills/msq-develop/SKILL.md`
- `docs/hotfixes/H05-msq-develop-run-recursion.md`
- `backlog.yaml` (spec de feat-15 atualizada com regras anti-recursao)
