 ...
 * failed/gates/tokens read at a glance without opening the Cost Dashboard.
 * Tokens are the current period (7d), not all-time (item 7) — all-time and
 * per-repo/feature breakdown stay in the Cost Dashboard (`d`).
 */
function StatsBarComponent({ done, todo, execution, failed, gatesPending, tokenStats, compact = false }: Props): React.ReactElement {
  ...
  return (
    <Box>
      ...
      <Text {...theme.role('muted')}> | </Text>
      <Text {...theme.statusTone('failed')}>{failed} failure</Text>
      <Text {...theme.role('muted')}> | </Text>
