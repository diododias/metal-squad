...
function ProjectCard({ project, repositories, actionResults, send, draft, onDraft, onSave }: { project: ProjectSummary; repositories: MsqWebState['repositories']; actionResults: ProjectsPageProps['actionResults']; send: ProjectsPageProps['send']; draft: ProjectDraft; onDraft: (draft: ProjectDraft) => void; onSave: () => void }): React.JSX.Element {
  ...
  return <Card style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
    ...
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}><StatusPill status={health.status} label={health.label} spinner={false} /><StatusPill status={project.activeRuns > 0 ? 'running' : 'not_started'} label={`${String(project.activeRuns)} active runs`} spinner={false} /></div>
    ...
    {draft.error && <div role="alert" style={errorStyle}>{draft.error}{isRevisionConflictMessage(draft.error) && ' Your draft is preserved; reload the current values and reapply it.'}</div>}
