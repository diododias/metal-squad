...
export function ProjectDetailPage({ state, projectId, send, actionResults, onBack }: {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
    ...
    <main style={{ overflow: 'auto', padding: 20, display: 'grid', gap: 16 }}>
      ...
      <section><h2 style={heading}>Epics</h2>{epics.length ? epics.map((epic) => <EpicCard key={epic.epicId} epic={epic} state={state} send={send} actionResults={actionResults} page={pageByEpic[epic.epicId] ?? 0} onPage={(page) => setPageByEpic((current) => ({ ...current, [epic.epicId]: page }))} />) : <Card><p style={muted}>No Epics yet.</p></Card>}</section>
    </main>
  </div>;
}

function EpicCard({ epic, state, send, actionResults, page, onPage }: { epic: EpicRow; state: MsqWebState; send: (message: WebSocketClientMessage) => void; actionResults: Record<string, Extract<WebSocketServerMessage, { type: 'action:result' }>>; page: number; onPage: (page: number) => void }): React.JSX.Element {
