...
export function EpicEditor({ epic, completedWorkItems, totalWorkItems, send, actionResults, requestId, onSaved }: EpicEditorProps): React.JSX.Element {
  ...
  return <section aria-label={`Edit Epic ${epic.title}`} style={{ display: 'grid', gap: 10 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap', alignItems: 'start' }}>
      ...
      <StatusPill status={epic.status === 'done' ? 'done' : epic.status === 'in_progress' ? 'running' : epic.status === 'in_review' ? 'in_review' : epic.status === 'archived' ? 'archived' : 'not_started'} label={epic.status} spinner={false} />
    </div>
    ...
    <div style={tags}><Tag>derived progress: {completedWorkItems}/{totalWorkItems}</Tag><Tag>status follows Work Item execution</Tag></div>
